...
#include "libcfile_narrow_split_string.h"
#include "libcfile_narrow_string.h"
#include "libcfile_path.h"
#include "libcfile_wide_split_string.h"
#include "libcfile_wide_string.h"

#if defined( WINAPI )
enum LIBCFILE_PATH_TYPES
{
	LIBCFILE_PATH_TYPE_ABSOLUTE,
	LIBCFILE_PATH_TYPE_DEVICE,
	LIBCFILE_PATH_TYPE_EXTENDED_LENGTH,
	LIBCFILE_PATH_TYPE_RELATIVE,
	LIBCFILE_PATH_TYPE_UNC
};
 ...
 */
int libcfile_path_change_directory(
     const char *directory_name,
     libcerror_error_t **error )
{
	static char *function = "libcfile_path_change_directory";
	DWORD error_code      = 0;
 ...
 * Returns 1 if successful or -1 on error
 */
int libcfile_path_get_current_working_directory(
     char **current_working_directory,
     size_t *current_working_directory_size,
     libcerror_error_t **error )
{
	static char *function                     = "libcfile_path_get_current_working_directory";
 ...
 */
int libcfile_path_get_full_path(
     const char *path,
     size_t path_length,
     char **full_path,
     ...
     libcerror_error_t **error )
{
	libcfile_narrow_split_string_t *current_directory_split_string = NULL;
	libcfile_narrow_split_string_t *path_split_string              = NULL;
	...
	static char *function                                          = "libcfile_path_get_full_path";
	...
	uint8_t path_type                                              = LIBCFILE_PATH_TYPE_RELATIVE;
	...
	if( path_length >= 2 )
	{
		   ...
		   && ( path[ 0 ] <= 'z' ) ) ) )
		{
			...
			if( ( path_length >= 3 )
			 && ( path[ 2 ] == '\\' ) )
			{
				path_type                  = LIBCFILE_PATH_TYPE_ABSOLUTE;
		      ...
		      && ( path[ 1 ] == '\\' ) )
		{
			 ...
			 && ( path[ 3 ] == '\\' ) )
			{
				if( path[ 2 ] == '.' )
				{
					path_type = LIBCFILE_PATH_TYPE_DEVICE;
				}
				else
				{
					path_type = LIBCFILE_PATH_TYPE_EXTENDED_LENGTH;
				}
			}
			else
			{
				...
				path_type          = LIBCFILE_PATH_TYPE_UNC;
				volume_name        = &( path[ 2 ] );
	 ...
	 */
	if( ( path_type != LIBCFILE_PATH_TYPE_DEVICE )
	 && ( path_type != LIBCFILE_PATH_TYPE_EXTENDED_LENGTH )
	 && ( path_type != LIBCFILE_PATH_TYPE_UNC ) )
	{
		...
		if( volume_name != NULL )
		{
			if( libcfile_path_get_current_working_directory(
			     &current_working_directory,
			     &current_working_directory_size,
			     error ) != 1 )
			{
			...
			if( libcfile_path_change_directory(
	...
	{
		if( libcfile_narrow_string_split(
	...
	*full_path_size = 4;
	 ...
	 */
	if( path_type == LIBCFILE_PATH_TYPE_RELATIVE )
	{
	...
	{
		if( libcfile_narrow_split_string_get_number_of_segments(
		     current_directory_split_string,
	...
	for( path_segment_index = 0;
	     ...
	     path_segment_index++ )
	{
		if( libcfile_narrow_split_string_get_segment_by_index(
		     path_split_string,
		     path_segment_index,
		     &path_string_segment,
		 ...
		 && ( path_string_segment[ 1 ] == '.' ) )
		{
			 ...
			 && ( last_used_path_segment_index == -1 ) )
			{
				...
				*full_path_size -= current_directory_string_segment_size;
				...
				if( libcfile_narrow_split_string_set_segment_by_index(
				     current_directory_split_string,
				     current_directory_segment_index,
				     NULL,
				     0,
	...
	*full_path = libcstring_narrow_string_allocate(
	...
	if( libcfile_narrow_split_string_free(
	     &path_split_string,
	     error ) != 1 )
	{
		libcerror_error_set(
...
on_error:
	...
	*full_path_size = 0;
	...
	{
		libcfile_narrow_split_string_free(
		 &path_split_string,
		 NULL );
	}
	if( current_directory_split_string != NULL )
     ...
     libcerror_error_t **error )
{
	 ...
	 * a directory separator
	 */
	if( path_type == LIBCFILE_PATH_TYPE_ABSOLUTE )
	{
		*full_path_size = 1;
	}
	/* If the path is relative
	 ...
	 * a directory separator, if necessary
	 */
	else if( path_type == LIBCFILE_PATH_TYPE_RELATIVE )
	{
...
int libcfile_path_join(
     ...
     libcerror_error_t **error )
{
	static char *function = "libcfile_path_join";
	size_t path_index     = 0;
	...
	*path_size = directory_name_length + filename_length + 1;
	...
	if( directory_name[ directory_name_length ] != (char) LIBCFILE_PATH_SEPARATOR )
	{
		*path_size += 1;
	}
	*path = libcstring_narrow_string_allocate(
	...
	( *path )[ path_index++ ] = (char) LIBCFILE_PATH_SEPARATOR;
 ...
 * Returns 1 if successful or -1 on error
 */
int libcfile_path_make_directory(
     const char *directory_name,
     libcerror_error_t **error )
{
	static char *function = "libcfile_path_make_directory";
	DWORD error_code      = 0;
 ...
 * Returns 1 if successful or -1 on error
 */
int libcfile_path_sanitize(
     char *path,
     size_t *path_size,
     libcerror_error_t **error )
{
	static char *function = "libcfile_path_sanitize";
	size_t path_index     = 0;
 ...
 * Returns 1 if successful or -1 on error
 */
int libcfile_path_sanitize_filename(
     char *filename,
     size_t *filename_size,
     libcerror_error_t **error )
{
	static char *function = "libcfile_path_sanitize_filename";
 ...
 * Returns 1 if successful or -1 on error
 */
int libcfile_path_change_directory_wide(
     const wchar_t *directory_name,
     libcerror_error_t **error )
{
	static char *function = "libcfile_path_change_directory_wide";
	DWORD error_code      = 0;
 ...
 * Returns 1 if successful or -1 on error
 */
int libcfile_path_get_current_working_directory_wide(
     wchar_t **current_working_directory,
     size_t *current_working_directory_size,
     libcerror_error_t **error )
{
	static char *function                     = "libcfile_path_get_current_working_directory_wide";
	DWORD safe_current_working_directory_size = 0;
	DWORD error_code                          = 0;
 ...
 * Returns 1 if succesful or -1 on error
 */
int libcfile_path_get_full_path_wide(
     const wchar_t *path,
     size_t path_length,
     wchar_t **full_path,
     size_t *full_path_size,
     libcerror_error_t **error )
{
	libcfile_wide_split_string_t *current_directory_split_string = NULL;
	libcfile_wide_split_string_t *path_split_string              = NULL;
	...
	static char *function                                        = "libcfile_path_get_full_path_wide";
	size_t current_directory_name_index                          = 0;
	size_t current_directory_size                                = 0;
	size_t current_directory_string_segment_size                 = 0;
	size_t current_working_directory_size                        = 0;
	 ...
	 && ( path_type != LIBCFILE_PATH_TYPE_UNC ) )
	{
		...
		if( volume_name != NULL )
		{
			if( libcfile_path_get_current_working_directory_wide(
			     &current_working_directory,
			     &current_working_directory_size,
			     error ) != 1 )
			{
			...
			if( libcfile_path_change_directory_wide(
			     change_volume_name ) != 1 )
			{
				libcerror_error_set(
				 error,
	...
	*full_path_size = 4;
	...
	if( current_directory_split_string != NULL )
	{
		if( libcfile_wide_split_string_get_number_of_segments(
		     current_directory_split_string,
		     &current_directory_number_of_segments,
		     error ) != 1 )
		{
	...
	for( path_segment_index = 0;
	     ...
	     path_segment_index++ )
	{
		if( libcfile_wide_split_string_get_segment_by_index(
		     path_split_string,
		     path_segment_index,
		     &path_string_segment,
		     &path_string_segment_size,
		 ...
		 && ( path_string_segment[ 1 ] == (wchar_t) '.' ) )
		{
			 ...
			 && ( last_used_path_segment_index == -1 ) )
			{
				...
				*full_path_size -= current_directory_string_segment_size;
				...
				if( libcfile_wide_split_string_set_segment_by_index(
				     current_directory_split_string,
				     current_directory_segment_index,
				     NULL,
				     0,
	...
	*full_path = libcstring_wide_string_allocate(
	...
	if( libcfile_wide_split_string_free(
	     &path_split_string,
	     error ) != 1 )
	{
		libcerror_error_set(
...
on_error:
	...
	*full_path_size = 0;
	...
	if( path_split_string != NULL )
	{
		libcfile_wide_split_string_free(
		 &path_split_string,
		 NULL );
	}
	if( current_directory_split_string != NULL )
...
int libcfile_path_join_wide(
     ...
     libcerror_error_t **error )
{
	static char *function = "libcfile_path_join_wide";
	size_t path_index     = 0;
	...
	*path_size = directory_name_length + filename_length + 1;
	...
	if( directory_name[ directory_name_length ] != (wchar_t) LIBCFILE_PATH_SEPARATOR )
	{
		*path_size += 1;
	}
	*path = libcstring_wide_string_allocate(
	...
	( *path )[ path_index++ ] = (wchar_t) LIBCFILE_PATH_SEPARATOR;
 ...
 * Returns 1 if successful or -1 on error
 */
int libcfile_path_make_directory_wide(
     const wchar_t *directory_name,
     libcerror_error_t **error )
{
	static char *function = "libcfile_path_make_directory_wide";
	DWORD error_code      = 0;
 ...
 * Returns 1 if successful or -1 on error
 */
int libcfile_path_sanitize_wide(
     wchar_t *path,
     size_t *path_size,
     libcerror_error_t **error )
{
	static char *function = "libcfile_path_sanitize_wide";
	size_t path_index     = 0;
 ...
 * Returns 1 if successful or -1 on error
 */
int libcfile_path_sanitize_filename_wide(
     wchar_t *filename,
     size_t *filename_size,
     libcerror_error_t **error )
{
	static char *function = "libcfile_path_sanitize_filename_wide";
	size_t filename_index = 0;
